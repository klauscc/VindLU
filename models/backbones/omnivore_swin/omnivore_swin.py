...
from .utils import load_and_broadcast_checkpoint_list
...
class BasicLayer(nn.Module):
    ...
    def forward(self, x, use_checkpoint=False, H=None, W=None, use_seg=False, return_predownsample=False):
        """Forward function.
        Args:
        ...
        pre_x = rearrange(x, "b d h w c -> b c d h w")
        if self.downsample is not None:
            x = self.downsample(x)
        x = rearrange(x, "b d h w c -> b c d h w")

        if return_predownsample:
            return x, pre_x
        else:
            return x
...
class PatchEmbed3D(nn.Module):
    ...
    def __init__(
        ...
        if additional_variable_channels:
            ...
            self.var_proj = [
                nn.Conv3d(x, embed_dim, kernel_size=patch_size, stride=patch_size) for x in additional_variable_channels
            ]
...
class SwinTransformer3D(nn.Module):
    ...
    def __init__(
        ...
        if depth_mode is not None:
            ...
            if depth_mode in ["separate_d_tokens", "summed_rgb_d_tokens"]:
                depth_chans = 1
                assert depth_patch_embed_separate_params, "separate tokenization needs separate parameters"
            ...
            if depth_patch_embed_separate_params:
                ...
                logging.info("Certain channels of patch projection may not be used in forward pass")
                logging.info("Make sure config.DISTRIBUTED.FIND_UNUSED_PARAMETERS is set to True")
                self.patch_embed = PatchEmbed3D(
                    patch_size=patch_size,
                    in_chans=3,
                    embed_dim=embed_dim,
    ...
    def inflate_weights(self, logger):
        ...
        if "classy_state_dict" in checkpoint:
            ...
            state_dict = checkpoint["classy_state_dict"][self.pretrained_model_key]["model"]["trunk"]
        else:
        ...
        state_dict["patch_embed.proj.weight"] = (
            state_dict["patch_embed.proj.weight"].repeat(1, 1, self.patch_size[0], 1, 1) / self.patch_size[0]
        )
        if "depth_patch_embed.proj.weight" in state_dict and state_dict["depth_patch_embed.proj.weight"].ndim == 4:
            state_dict["depth_patch_embed.proj.weight"] = state_dict["depth_patch_embed.proj.weight"].unsqueeze(2)
    ...
    def load_and_interpolate_3d_weights(self, logger):
        ...
        assert pretrained_window_size[0] == self.window_size[0], "Interpolating along time not supported"
        ...
        for k in relative_position_bias_table_keys:
            ...
            L1, nH1 = relative_position_bias_table_pretrained.size()
            L2, nH2 = relative_position_bias_table_current.size()
            L2 = (2 * self.window_size[0] - 1) * (2 * self.window_size[1] - 1) * (2 * self.window_size[2] - 1)
            if nH1 != nH2:
                logger.warning(f"Error in loading {k}, passing")
            else:
                if L1 != L2:
                    pretrained_bias = relative_position_bias_table_pretrained.view(T1, S11, S12, nH1)
                    ...
                    relative_position_bias_table_pretrained_resized = (
                        relative_position_bias_table_pretrained_resized.permute(0, 2, 3, 1)
                    )
    ...
    def init_weights(self, pretrained=None):
        ...
        if isinstance(self.pretrained, str) or isinstance(self.pretrained, list):
            ...
            if self.pretrained2d:
                ...
                raise ValueError("Use VISSL loading for this. This code " "is only for Swin inflation.")
                # # Directly load 3D model.
                # load_checkpoint(self, self.pretrained, strict=False, logger=logger)
        elif self.pretrained is None:
            self.apply(_init_weights)
        else:
            raise TypeError(f"pretrained must be a str or None but found: {type(self.pretrained)}")
    ...
    def forward(
        ...
        for i, layer in enumerate(self.layers):
            x, pre_x = layer(x.contiguous(), use_checkpoint=use_checkpoint, return_predownsample=True)
            stage_outputs.append(pre_x)
